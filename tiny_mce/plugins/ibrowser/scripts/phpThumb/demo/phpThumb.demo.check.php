...
echo '</th><td>ImageMagick is faster than GD, can process larger images without PHP memory_limit issues, can resize animated GIFs. phpThumb can perform basic resizing with ImageMagick only, even if GD is not available.</td></tr>';
...
echo '<td>SAPI mode preferred to CGI mode. If not working in "apache" (SAPI) mode, <i>apache_lookup_uri()</i> will not work.</td></tr>';
...
$FunctionsExist = array(
	...
	'memory_get_usage'      => array('green',     'mostly used for troubleshooting.'),
	...
	'ImageCreateFromWBMP'   => array('orange',    'required for WBMP source images using GD.'),
...
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @get_cfg_var('magic_quotes_runtime')).'</th><th bgcolor="';
if (@ini_get('magic_quotes_runtime')) {
	echo 'red';
} else {
	echo 'lime';
}
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @ini_get('magic_quotes_runtime')).'</th>';
echo '<td>This setting is evil. Turn it off.</td></tr>';
...
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @ini_get('magic_quotes_runtime')).'</th>';
echo '<td>This setting is bad. Turn it off, if possible. phpThumb will attempt to work around it if it is enabled</td></tr>';
...
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @get_cfg_var('safe_mode')).'</th><th bgcolor="';
if (@get_cfg_var('safe_mode')) {
	echo 'yellow';
} else {
	echo 'lime';
}
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @ini_get('safe_mode')).'</th>';
echo '<td>Best if off. Calls to ImageMagick will be disabled if on (limiting max image resolution, no animated GIF resize).</td></tr>';
...
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @get_cfg_var('allow_url_fopen')).'</th><th bgcolor="';
if (@ini_get('allow_url_fopen')) {
	echo 'lime';
} else {
	echo 'yellow';
}
echo '">'.$phpThumb->phpThumbDebugVarDump((bool) @ini_get('allow_url_fopen')).'</th>';
echo '<td>Best if on. HTTP source images will be unavailable if disabled and CURL is unavailable.</td></tr>';
...
echo '">'.eregi_replace('(shell_exec|system|passthru|exec)(,|$)', '<span style="background-color: red;">\\1</span>(\\2)', $disable_functions).'</th><th bgcolor="';
...
echo '">'.eregi_replace('(shell_exec|system|passthru|exec)(,|$)', '<span style="background-color: red;">\\1</span>(\\2)', $disable_functions).'</th>';
