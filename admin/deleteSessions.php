      ...
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" -->
		...
		if (isset($_POST['confirm']) && $_POST['confirm']==1) {
			for ($i = 0; $i < count($_POST['delete']); $i++) {
				...
				$myRsRes=mysql_query($sessionsSQL, $conn)
					or die("Invlaid Query: " . $sessionsSQL . " - " . mysql_error());
				$myRs=mysql_fetch_assoc($myRsRes);
				$answersSQL="DELETE FROM Answers WHERE SessionID=" . $myRs['ID'];
				$answersRes=mysql_query($answersSQL, $conn)
					or die("Invalid Query: " . $answersSQL . " - " . mysql_error());
				$sessionsDel="DELETE FROM Sessions WHERE ID=" . $_POST['delete'][$i];
				$result=mysql_query($sessionsDel, $conn)
					or die("Invalid Query: " . $sessionsDel . " - " . mysql_error());
			}
			redirect_to("currentSessions.php");
			exit;
		}
