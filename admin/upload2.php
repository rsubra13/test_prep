    ...
    <tr> 
      <td align="center" valign="top">		<div align="left"><!-- InstanceBeginEditable name="Content Area" --> 
		...
		if ($_SESSION['loggedIn'] != "1") redirect_to("index.php");
			if (!$_POST) {
			echo("Select up to 10 files you want to upload by clicking the Browse button.<br><br>");
			}
			$numoffile = 10; 
			$form=1;
			// Fix path of your file to be uploaded, don't forget to CHMOD 777 to this folder 
			$file_dir  = realpath("../test-images/" . $_POST['uploadFolder']) . "/";
  			if ($_POST) { 
    			for ($i=0;$i<$numoffile;$i++) { 
      			if (trim($_FILES['myfiles']['name'][$i])!="") { 
        			$newfile = $file_dir.$_FILES['myfiles']['name'][$i]; 
        			move_uploaded_file($_FILES['myfiles']['tmp_name'][$i], $newfile); 
        			echo($_FILES['myfiles']['name'][$i] . " uploaded (" . strval($_FILES['myfiles']['size'][$i] / 1024) . "kb)<br>");
        			$j++; 
      			} 
    		} 
  		} 
  		if (isset($j)&&$j>0) {
  			print $j . " file(s) uploaded.<br>";
  			$form=0;
  		}
  		if ($form=1) {
	  		print "<form method='post' enctype='multipart/form-data'>"; 
	  		for($i=0;$i<$numoffile;$i++) {
	  			// echo("Image or movie " . strval($i + 1) . ":<br>");
	    		print "<input type='file' name='myfiles[]' size='30'><br>"; 
	  		} 
	  		print "<input type='submit' name='action' value='Upload'>";
	  		print "<input type='hidden' name='uploadFolder' value='" . $_REQUEST['uploadFolder'] . "'";
	  		print "</form>"; 
	  	}
		?>  
