...
?>
    ...
    <tr> 
	  ...
	  <?php if ($_SESSION['loggedIn'] != "1") redirect_to("index.php") ?>
        <p>Are you sure you want to delete the following files?</p>
        <form name="form1" method="post" action="delete.php?uploadFolder=<?=$_REQUEST['uploadFolder']?>">
		  <p>
            <?php
		if (count($_POST['fileToDel']) < 1) {
			redirect_to("remove.php");
			exit;
		}
		if ($_POST['No'] == "No") {
			redirect_to("remove.php");
		...
		if ($_POST['confirm'] == "1") {
			$fileToDel=realpath("../test-images/" . $_REQUEST['uploadFolder'] . "/" . $_POST['fileToDel']);
			...
			echo("<input name='fileToDel' type='hidden' value='" . $_POST['fileToDel'] . "'>" . $_POST['fileToDel'] . "<BR>");
		?>
          </p>
          <p>
            <input name="Yes" type="submit" id="delete" value="Yes">
            <input name="No" type="submit" id="delete" value="No">
